...
function PostData() {
	...
	return (
		<div className='md:px-[50px] px-[20px] py-[30px] mx-auto max-w-[1000px]'>
			<Link to='/post' className='bg-[#f4f4f4] px-[21px] py-[7px] font-medium text-[16px]'>
				Back To Posts
			</Link>
			...
			<div className='flex flex-col md:flex-row gap-[50px] items-center border-gray-300 border-[1px] rounded-[4px] px-[20px] py-[10px] mx-auto max-w-[1000px] mb-[20px] mt-[30px]'>
				<div className='avatar flex justify-center items-center flex-col'>
					<div className='max-w-[170px] min-w-[170px] mx-auto'>
						<img src={user.avatar} alt='' className='w-full h-full rounded-[50%]' />
					</div>
					<h2>{user.name}</h2>
				</div>
				...
				<div>
					...
					<div className='flex items-center gap-[10px]'>
						<button className='h-[38px] w-[57px] bg-[#f4f4f4] flex justify-center items-center text-[20px]' onClick={() => handleLike(user._id)}>
							<BiSolidLike />
							{user.likes?.length > 0 && <span className='ml-1'>{user.likes.length}</span>}
						</button>

						<button className='h-[38px] w-[57px] bg-[#f4f4f4] flex justify-center items-center text-[20px]' onClick={() => handleUnlike(user._id)}>
							<BiSolidDislike />
						</button>
                        <Link to={`/post/${id}`} className='px-[21px] py-[7px] bg-primary text-white'>
                                 Discussion {comment.length > 0 && (<span className='bg-white text-primary rounded-md px-1'>{comment.length}</span>)}
                        </Link>
			...
			<form onSubmit={SubmitText} className='max-w-[1000px] mx-auto mb-[30px]'>
				...
				></textarea>
				<button className='text-[16px] px-[21px] py-[7px] bg-[#343a40] text-white rounded-[4px]'>Submit</button>
			</form>

			<div>
				{comment.map((data, index) => (
					<div key={index} className='flex flex-col md:flex-row gap-[50px] items-center border-gray-300 border-[1px] rounded-[4px] md:px-[20px] py-[10px] mx-auto max-w-[1000px] mb-[20px]'>
						<div className='avatar flex justify-center items-center flex-col'>
							<div className='max-w-[170px] min-w-[170px] mx-auto'>
